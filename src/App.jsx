...
function SplashScreen({ userName, isNew=false }) {
  ...
  return (
    <div className="c-screen" style={{
      ...
      background:"radial-gradient(ellipse at 50% 40%, #0a1f0f 0%, #060d08 55%, #020504 100%)",
      ...
      <style>{`
        ...
        /* ── 6. Ballon — rebond lent avec effet 3D ── */
        @keyframes ballIn {
          0%,55%  { transform:translateY(320px) scale(0.15) rotate(0deg);   opacity:0; }
          60%     { opacity:1; }
          /* 1er rebond haut — s'étire */
          70%     { transform:translateY(-60px)  scaleX(0.88) scaleY(1.18) rotate(200deg); opacity:1; }
          /* s'écrase en bas */
          76%     { transform:translateY(16px)   scaleX(1.18) scaleY(0.85) rotate(255deg); opacity:1; }
          /* 2e rebond */
          82%     { transform:translateY(-30px)  scaleX(0.92) scaleY(1.1)  rotate(295deg); opacity:1; }
          /* s'écrase */
          87%     { transform:translateY(8px)    scaleX(1.1)  scaleY(0.92) rotate(325deg); opacity:1; }
          /* 3e rebond léger */
          91%     { transform:translateY(-14px)  scaleX(0.96) scaleY(1.05) rotate(342deg); opacity:1; }
          /* stabilise */
          95%     { transform:translateY(4px)    scaleX(1.04) scaleY(0.97) rotate(354deg); opacity:1; }
          100%    { transform:translateY(0px)    scaleX(1)    scaleY(1)    rotate(360deg); opacity:1; }
        }

        /* Ombre ballon — s'écrase à chaque rebond */
        @keyframes shadowIn {
          0%,55%  { transform:scaleX(.05) scaleY(.3); opacity:0; }
          70%     { transform:scaleX(1.5) scaleY(.35); opacity:.55; }
          76%     { transform:scaleX(1.3) scaleY(.6);  opacity:.35; }
          82%     { transform:scaleX(1.3) scaleY(.4);  opacity:.5; }
          87%     { transform:scaleX(1.1) scaleY(.65); opacity:.32; }
          91%     { transform:scaleX(1.15) scaleY(.5); opacity:.42; }
          95%     { transform:scaleX(1.05) scaleY(.75);opacity:.3; }
      ...
      {/* Conteneur principal — anneaux + SVG parfaitement centrés ensemble */}
      <div style={{position:"relative", width:290, height:290, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0}}>

        {/* Anneaux — absolus par rapport au conteneur, parfaitement centrés */}
        <div style={{
          position:"absolute", width:326, height:326, borderRadius:"50%",
          border:"1px solid #00e67622",
          top:"50%", left:"50%", marginTop:"-163px", marginLeft:"-163px",
          animation:"ring1 16s linear infinite",
        }}/>
        <div style={{
          position:"absolute", width:272, height:272, borderRadius:"50%",
          border:"1px dashed #76ff031e",
          top:"50%", left:"50%", marginTop:"-136px", marginLeft:"-136px",
          animation:"ring2 11s linear infinite",
        }}/>

      {/* ═══ SVG LOGO SE CONSTRUIT ═══ */}
      <div className="c-glow" style={{position:"relative",width:290,height:290,flexShrink:0}}>
        <svg width="290" height="290" viewBox="0 0 290 290" xmlns="http://www.w3.org/2000/svg" style={{display:"block"}}>
          <defs>
